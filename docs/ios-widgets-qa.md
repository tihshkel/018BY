...
|--------|-------|--------|-------|-------------|-------------|
| Сводка (бывш. Быстрый доступ) | ✓ | ✓ | — | — | Inline |
| Альбомы | ✓ | ✓ | ✓ | ✓ (iPad) | — |
| Продолжить | ✓ | ✓ | ✓ | — | Rectangular |
| Важные даты | ✓ | ✓ | ✓ | — | Inline + Circular |
| До ПДР | ✓ | ✓ | ✓ | — | Circular + Rectangular |
...
5. **До ПДР** — с датой: **дни до ПДР** (крупно), неделя, триместр; без даты: «Укажите ПДР» → `paper-album-notifications`; с датой tap → альбом беременности
6. **Уведомления** — push за сегодня попадает в inbox → виджет показывает заголовок и текст; tap → **История уведомлений**
7. **Сводка** — число альбомов + дни до ПДР (+ ближайшая дата на medium); без CTA-кнопки «+»
