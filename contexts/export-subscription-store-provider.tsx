...
export function ExportSubscriptionStoreProvider({
  children,
}: {
  ...
  const purchase = useCallback(async (): Promise<boolean> => {
    ...
    return new Promise<boolean>((resolve) => {
      ...
      requestPurchase({
        request: {
          apple: { sku: EXPORT_PRINT_UNLOCK_SKU },
          google: { skus: [EXPORT_PRINT_UNLOCK_SKU] },
        },
        type: 'in-app',
      }).catch(() => {
        pendingPurchaseRef.current?.resolve(false);
  ...
  const value = useMemo<ExportSubscriptionContextValue>(
    ...
    [isSubscribed, isLoading, connected, priceLabel, purchase, restore, refresh]
  );
