...
export const preloadPriorityImages = async (): Promise<void> => {
  try {
    ...
    const preloadPromises = priorityImagesForPreload.map(async (imageSource) => {
      // Для require() модулей (number), используем Image.prefetch для предзагрузки в диск-кеш
      // Это гарантирует быструю загрузку при первом отображении
      try {
        if (typeof imageSource === 'number') {
          // Локальный ассет - предзагружаем в кеш expo-image
          // Создаем временный Image компонент для предзагрузки
    ...
    for (let i = 0; i < imagesToLoad.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(async (imageSource) => {
          try {
            if (typeof imageSource === 'number') {
              // Локальный ассет - предзагружаем в кеш
              await Image.prefetch(imageSource);
              return;
            }
