...
# -*- coding: utf-8 -*-
"""
Скрипт для конвертации PDF файлов из albums/diary/cover/in album в изображения.
Конвертирует все PDF файлы в папки с изображениями в той же директории.
"""
...
def main():
    ...
    in_album_dir = project_root / "albums" / "diary" / "cover" / "in album"
    
    if not in_album_dir.exists():
        print(f"Ошибка: Папка {in_album_dir} не найдена.")
        sys.exit(1)
    
    print("=" * 60)
    print("Конвертация PDF файлов в изображения")
    print("=" * 60)
    print(f"Исходная папка: {in_album_dir}")
    print()
    
    # Находим все PDF файлы в папке (не в подпапках)
    pdf_files = sorted([f for f in in_album_dir.glob("*.pdf") if f.is_file()])
    
    if not pdf_files:
        print("⚠ PDF файлы не найдены в папке cover/in album")
        sys.exit(0)
    ...
    # Спрашиваем об удалении PDF файлов
    if success_count > 0:
        print()
        response = input("Удалить PDF файлы после конвертации? (y/n): ").strip().lower()
        if response == 'y' or response == 'yes' or response == 'да':
            deleted_count = 0
            for pdf_path in pdf_files:
                try:
                    pdf_path.unlink()
                    print(f"  ✓ Удален: {pdf_path.name}")
                    deleted_count += 1
                except Exception as e:
                    print(f"  ✗ Ошибка при удалении {pdf_path.name}: {e}")
            print(f"\nУдалено PDF файлов: {deleted_count}")
        else:
