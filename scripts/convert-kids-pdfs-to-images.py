...
# -*- coding: utf-8 -*-
"""
Скрипт для конвертации всех PDF файлов в папке albums/kids в изображения.
...
def convert_pdf_to_images(pdf_path: Path, output_dir: Path, dpi: int = 300):
    ...
    print(f"Обработка: {pdf_path.relative_to(pdf_path.parents[2])}")
...
def main():
    ...
    # Рекурсивно находим все PDF файлы в папке kids
    pdf_files = sorted(kids_dir.rglob("*.pdf"))
    
    if not pdf_files:
        print("⚠ PDF файлы не найдены в папке albums/kids")
        sys.exit(0)
